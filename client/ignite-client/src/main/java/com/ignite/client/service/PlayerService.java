package com.ignite.client.service;

import com.ignite.client.model.Player;
...
import static com.ignite.client.constants.CacheConstant.PLAYER;

@Service
@Slf4j
public class PlayerService {
  private final CacheManager cacheManager;
  private final String password = "aab";
  ...
  public List<Player> scanQuerySearch(String text) {
    ...
    IgniteCache<Long, Player> playerCache = Ignition.ignite().getOrCreateCache(PLAYER);
    ScanQuery<Long, Player> scanQuery =
        new ScanQuery<>((k, v) -> v.getTeam().equalsIgnoreCase(text));
  ...
  public List<Player> sqlQuerySearch(String text) {
    ...
    Collection<QueryEntity> queryEntities =
        playerCache.getConfiguration(CacheConfiguration.class).getQueryEntities();
    String tableName =
        queryEntities.stream()
            .findFirst()
            .orElseThrow(() -> new IllegalArgumentException(PLAYER + "doesn't exists"))
            .getTableName();
    SqlFieldsQuery sqlQuery =
        new SqlFieldsQuery("select * from " + tableName + " where team='" + text + "'");
    log.info("Query :: " + sqlQuery.getSql());
    List<List<?>> records = playerCache.query(sqlQuery).getAll();
    return records.stream()
        .map(objects -> convert((List<Object>) objects))
        .collect(Collectors.toList());
  }

  private Player convert(List<Object> values) {
    return Player.builder()
        .id((Long) values.get(0))
        .name(String.valueOf(values.get(1)))
        .team(String.valueOf(values.get(2)))
        .salary(Double.parseDouble(values.get(3).toString()))
        .build();
  }
